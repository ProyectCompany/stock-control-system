...
export const generateStockPDF = (products: Product[], user: UserProfile) => {
  ...
  // Header Colors & Style (Dark Charcoal & Emerald Accent)
  doc.setFillColor(45, 41, 38); // Dark Charcoal
  ...
  doc.setTextColor(180, 80, 0); // Amber warning
  doc.text(`Alertas Stock Bajo: ${lowStockCount}`, 145, 48);
  doc.setTextColor(190, 40, 40); // Red warning
  ...
  // Save the PDF
  const filename = `Stock_${user.businessName.replace(/\s+/g, '_')}_${now.toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
};
